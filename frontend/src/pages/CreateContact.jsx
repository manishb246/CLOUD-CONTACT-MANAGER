...
export default function CreateContact() {
  ...
  const handleChange = (e) => {
    ...
    const nextValue = name === "mobileNo" ? value.replace(/\D/g, "").slice(0, 10) : value;
  ...
  const handleSubmit = async (e) => {
    ...
    const nextErrors = validateContactForm(sanitizedForm);
    ...
    try {
      await addContact(sanitizedForm);
      navigate("/dashboard");
    } catch (err) {
  ...
  return (
    <main className="max-w-2xl mx-auto px-3 sm:px-6 py-4 sm:py-8 animate-in zoom-in-95 fade-in duration-500">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4">
          <div className="space-y-4">
            ...
            <div className="group">
              ...
              <input type="url" name="url" placeholder="https://example.com/photo.jpg" value={form.url} onChange={handleChange}
                className="w-full border border-gray-200 bg-gray-50 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 focus:bg-white transition-all" />
              {errors.url && <p className="text-xs text-red-500 mt-1 ml-1">{errors.url}</p>}
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="group">
                <label className="text-xs font-bold text-gray-400 uppercase ml-1 mb-1 block">Mobile</label>
                <input type="tel" name="mobileNo" placeholder="10-digit phone number" value={form.mobileNo} onChange={handleChange} required
                  inputMode="numeric" pattern="\d{10}" maxLength={10}
                  className="w-full border border-gray-200 bg-gray-50 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 focus:bg-white transition-all" />
                {errors.mobileNo && <p className="text-xs text-red-500 mt-1 ml-1">{errors.mobileNo}</p>}
              </div>
              <div className="group">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="group">
                ...
                <input type="text" name="title" placeholder="e.g. Software Engineer" value={form.title} onChange={handleChange}
                  className="w-full border border-gray-200 bg-gray-50 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 focus:bg-white transition-all" />
              </div>
            </div>
